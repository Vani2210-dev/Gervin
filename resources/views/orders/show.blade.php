...
@section('content')
        ...
        return $s->items;
            ...
            <div class="card p-0 rounded-xl border-0 overflow-hidden shadow-sm bg-white">
                <div class="card-header border-b border-neutral-200 bg-white py-4 px-6 flex items-center gap-2">
                    ...
                    @if($paidPct >= 100)
                        <span class="text-xs px-2 py-0.5 rounded-full bg-success-100 text-success-600 font-semibold">Đã thanh
                            toán đủ</span>
                    @elseif($totalPaid > 0)
                        <span
                            class="text-xs px-2 py-0.5 rounded-full bg-warning-100 text-warning-600 font-semibold">{{ $paidPct }}%</span>
                    @endif
                </div>
                <div class="p-5 space-y-4">
                    ...
                    <div class="grid grid-cols-3 gap-2 text-center">
                        <div class="bg-neutral-50 rounded-lg p-3">
                            ...
                            <div class="text-sm font-bold text-neutral-800">
                                {{ number_format(round($totalAmount, -3), 0, ',', '.') }}₫</div>
                        </div>
                        ...
                        <div class="rounded-lg p-3 {{ $totalDebt > 0 ? 'bg-danger-50' : 'bg-success-50' }}">
                            <div
                                class="text-xs font-medium mb-1 {{ $totalDebt > 0 ? 'text-danger-500' : 'text-success-500' }}">
                                Còn nợ</div>
                            <div class="text-sm font-bold {{ $totalDebt > 0 ? 'text-danger-600' : 'text-success-600' }}">
                    ...
                    @if($payments->count() > 0)
                        <div>
                            <div class="text-xs font-semibold text-neutral-500 uppercase tracking-wider mb-2">Lịch sử thanh toán
                            </div>
                                    ...
                                    <div class="flex items-start gap-2 bg-neutral-50 rounded-lg px-3 py-2 group">
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center gap-2 flex-wrap">
                                                <span
                                                    class="text-xs font-semibold text-neutral-700">{{ $pmt->payment_date->format('d/m/Y') }}</span>
                                                <span
                                            ...
                                            <div class="text-sm font-bold text-success-600 mt-0.5">
                                                +{{ number_format($pmt->amount, 0, ',', '.') }}₫</div>
                                            @if($pmt->note)
                                        ...
                                        <div class="flex-shrink-0 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <form method="POST"
                                                action="{{ route('orders.payments.destroy', [$acrylicOrder, $pmt]) }}"
                    ...
                    <div>
                        ...
                        <div id="add-payment-form" class="hidden">
                            <form method="POST" action="{{ route('orders.payments.store', $acrylicOrder) }}"
                                class="space-y-3">
                                @csrf
                                <div class="grid grid-cols-2 gap-2">
                                    <div>
                                        <label class="text-xs font-semibold text-neutral-500 block mb-1">Ngày <span
                                    ...
                                    <div>
                                        <label class="text-xs font-semibold text-neutral-500 block mb-1">Số tiền (₫) <span
                                                class="text-danger-500">*</span></label>
                                        <input type="number" name="amount" min="0" step="any" placeholder="0"
                                            class="form-control form-control-sm rounded-lg border-neutral-300 focus:border-violet-400 focus:ring-violet-400 text-sm"
                                            required>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <select name="payment_method"
                                        class="form-select form-select-sm rounded-lg border-neutral-300 focus:border-violet-400 focus:ring-violet-400 text-sm">
    ...
    {{-- ===== EDIT PAYMENT MODAL ===== --}}
    <div id="edit-payment-backdrop" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.4); z-index:2000;"
        onclick="closeEditPayment()"></div>
    <div id="edit-payment-modal" style="display:none; position:fixed; top:50%; left:50%; transform:translate(-50%,-50%);
               width:min(420px,95vw); background:#fff; border-radius:16px;
               box-shadow:0 20px 60px rgba(0,0,0,0.3); z-index:2001; overflow:hidden;">
        <div
            style="background:linear-gradient(135deg,#7c3aed,#6d28d9); padding:18px 24px; display:flex; align-items:center; justify-content:space-between;">
        ...
        <form id="edit-payment-form" method="POST"
            ...
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">
                <div>
                    <label style="font-size:12px; font-weight:600; color:#374151; display:block; margin-bottom:5px;">Ngày
                ...
                <div>
                    <label style="font-size:12px; font-weight:600; color:#374151; display:block; margin-bottom:5px;">Số tiền
                        (₫) <span style="color:#ef4444;">*</span></label>
                    <input id="ep-amount" type="number" name="amount" min="0" step="any" required
                        style="width:100%; padding:9px 12px; border:1.5px solid #d1d5db; border-radius:8px; font-size:13px; outline:none; box-sizing:border-box;">
                </div>
            </div>
            <div>
                <label style="font-size:12px; font-weight:600; color:#374151; display:block; margin-bottom:5px;">Hình
                    thức</label>
            ...
            <div>
                <label style="font-size:12px; font-weight:600; color:#374151; display:block; margin-bottom:5px;">Ghi
                    chú</label>
